...
RUN dotnet build "./src/Wiz.Devedor.API/Wiz.Devedor.API.csproj" -c Debug
CMD dotnet run --project src/Wiz.Devedor.API/Wiz.Devedor.API.csproj
...
COPY . .
RUN dotnet restore . -s $nuget_endpoint -s "https://api.nuget.org/v3/index.json"
RUN dotnet build "./src/Wiz.Devedor.API/Wiz.Devedor.API.csproj" -c Release

FROM build AS publish
RUN dotnet publish "./src/Wiz.Devedor.API/Wiz.Devedor.API.csproj" -c Release -o /publish

FROM mcr.microsoft.com/dotnet/core/aspnet:3.1-buster-slim AS production
ENV ASPNETCORE_ENVIRONMENT=Production
WORKDIR /app
COPY --from=publish /publish .
ENTRYPOINT ["dotnet", "Wiz.Devedor.API.dll"]
EXPOSE 80